...
const View = () => {
  ...
  const toggleStatus = async () => {
    const newStatus = enumerator.status === "active" ? "inactive" : "active"
    setEnumerator(prev => ({ ...prev, status: newStatus }))
    await axios.post(route('api.admin.update.enumerator.status'), {
      enumerator_id,
      status: newStatus
    })
  }
                  ...
                  <CardBody className="space-y-4 max-sm:p-4">
                    ...
                    <div className="flex justify-between items-center">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs font-normal">Once you deactivate this enumerator, they will no longer be able to log in.</p>
                      </div>
                      <Switch checked={enumerator.status === "active"} onChange={toggleStatus} color="green" label={enumerator.status} labelProps={{ className: "font-normal capitalize text-sm" }} />
                    </div>
                  </CardBody>
                </Card>
              </TabPanel>
